...
public class ProgrammaticConfigurationTest {
	...
	public void testManualGlobalConfiguration() {
		GlobalConfiguration globalConfig = new GlobalConfigurationBuilder()
				.globalJmxStatistics().disable().build();

		Configuration config = new ConfigurationBuilder().eviction()
				.maxEntries(20000).strategy(EvictionStrategy.LIRS).expiration()
				.wakeUpInterval(5000L).maxIdle(120000L).jmxStatistics()
				.disable().build();

		CacheContainer container = new DefaultCacheManager(globalConfig, config);

		assertEquals(container.getCache().getCacheConfiguration().eviction()
				.maxEntries(), 20000);
		assertEquals(container.getCache().getCacheConfiguration().eviction()
				.strategy(), EvictionStrategy.LIRS);
		assertEquals(container.getCache().getCacheConfiguration().expiration()
