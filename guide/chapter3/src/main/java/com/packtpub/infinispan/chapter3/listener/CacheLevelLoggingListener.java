...
public class CacheLevelLoggingListener {
	...
	/*
	 * In this method we are using annotations to be notified when a cache entry
	 * is visitor, modified, loaded, invalidated or removed.
	 */
	@CacheEntryVisited
	@CacheEntryModified
	@CacheEntryLoaded
	...
	public void transactionRegistered(TransactionRegisteredEvent<?, ?> event) {
		log.infof("[Event %s] Transaction '{%s}' registered", event.getType(),
				event.getGlobalTransaction().getId());
		transactions.put(event.getGlobalTransaction(),
	...
	public void transactionCompleted(TransactionCompletedEvent<?, ?> event) {
		...
		log.infof("[Event %s] Transaction '{%s}' completed", event.getType(),
				event.getGlobalTransaction().getId());
	...
	/*
	 * Method to logging information when the ConsistentHash implementation in
	 * use by the DistributionManager.
	 */
	@TopologyChanged
	public void topologyChanged(TopologyChangedEvent<?, ?> event) {
		log.infof(
				"[Event %s] Cache entry Topology Changed. New Topology ID: %s",
				event.getType(), event.getNewTopologyId());
	}
}
