...
public class WeatherService {
	...
	@Inject @OpenWeatherCache
	private Cache<String, String> openWeatherCache;

	@Inject @WunderGroundCache
	...
	public void jcacheOperations(String country, String city, String weather){
		...
		logger.infof("Looking the weather for %s = %s", getKey(country, city), openWeatherInfo.getWeatherFor(getKey(country, city)));
		openWeatherInfo.createEntry(getKey(country, city), "The weather is good!!!!");
		logger.infof("Looking the weather again for %s = %s", getKey(country, city), openWeatherInfo.getWeatherFor(getKey(country, city)));
		openWeatherInfo.clearCache();
		logger.infof("Looking the weather after clean the cache s %s", getKey(country, city), openWeatherInfo.getWeatherFor(getKey(country, city)));
	}
