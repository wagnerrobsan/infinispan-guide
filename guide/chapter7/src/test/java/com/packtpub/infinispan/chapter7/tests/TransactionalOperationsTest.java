...
public class TransactionalOperationsTest extends BaseTest{
	...
	public void transactionWithLockingTest() throws Exception, SystemException {
		Cache<String, Guest> cache = manager
				.getCache("transactionCacheWithLocking");
		tm = cache.getAdvancedCache().getTransactionManager();
		assertEquals(Status.STATUS_NO_TRANSACTION, tm.getStatus());
		tm.begin();
		cache.put("wds.212", guest);
	...
	@Test
	public void transactionWithLockingAndDeadlockDetectionTest()
			throws Exception, SystemException {
		Cache<String, Guest> cache = manager
				.getCache("transactionCacheWithLockingAndDeadlockDetection");
		tm = cache.getAdvancedCache().getTransactionManager();
		assertEquals(Status.STATUS_NO_TRANSACTION, tm.getStatus());
		tm.begin();
		cache.put("wds.212", guest);
	...
	@Test
	public void transactionWithLockingAndWriteSkewCheckAndVersioningTest()
			throws Exception, SystemException {
		Cache<String, Guest> cache = manager
				.getCache("transactionCacheWithLockingWriteSkewCheckAndVersioning");
		tm = cache.getAdvancedCache().getTransactionManager();
		assertEquals(Status.STATUS_NO_TRANSACTION, tm.getStatus());
		tm.begin();
		cache.put("wds.212", guest);
	...
	public void CacheExample01Test() throws Exception, SystemException {
		Cache<String, Guest> cache = manager
				.getCache("transactionCacheExample01");
		tm = cache.getAdvancedCache().getTransactionManager();
		assertEquals(Status.STATUS_NO_TRANSACTION, tm.getStatus());
		tm.begin();
		cache.put("wds.212", guest);
	...
	public void transactionProgrammaticAPI() {
		...
		GlobalConfiguration gc = new GlobalConfigurationBuilder()
				.globalJmxStatistics().jmxDomain("Programmatic API Test")
				.enable().build();
		Configuration config = new ConfigurationBuilder()
				.transaction()
				.transactionMode(TransactionMode.TRANSACTIONAL)
				.transactionManagerLookup(new GenericTransactionManagerLookup())
				.syncRollbackPhase(false).syncCommitPhase(true)
				.cacheStopTimeout(30000l)
				.use1PcForAutoCommitTransactions(false).autoCommit(true)
				.useSynchronization(false).build();
	...
	public void CacheExampleVersioningTest() throws Exception, SystemException {
		Configuration config = new ConfigurationBuilder()
				...
				.transactionMode(TransactionMode.TRANSACTIONAL)
				.transactionManagerLookup(new GenericTransactionManagerLookup())
				.versioning().enable().scheme(VersioningScheme.SIMPLE).build();
